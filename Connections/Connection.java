...
public class Connection {
	...
	void connect() throws MalformedURLException, IOException {
		try {
			...
			HttpURLConnection conn = (HttpURLConnection) (new URL(connectUrl))
					.openConnection();
			conn.setDoOutput(true);// set to true because we also want to use
									// URL connection for output sometimes
			...
			if (conn.getResponseCode() != HttpURLConnection.HTTP_OK) {
				System.out
						.println("Wrong Response Code. There appears to be a problem");
	...
	String getDataContent(String newFile) throws MalformedURLException,
			IOException {
		String result = "";
		try {
	...
	public static void main(String args[]) throws MalformedURLException,
			IOException {
		/*
		 * here is an example for how the classes and objects work. Create a
		 * connection object and then use the various methods to retrieve data
		 */
